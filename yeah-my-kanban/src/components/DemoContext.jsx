...
export function MyComponent1() {
  ...
  const handleAddCount = () => {
    setCount(count + 1);
    // setCount 更新count, 当前组件重新渲染，使用 Context value 的 MyGrandchildComponent组件重新渲染是正常的，
    // 未使用Context value值的 MyChildComponent 不重新渲染，也是正常的
  };
  ...
  return (
    <MyContext.Provider value={state1}>
      <ul>
        <MyChildComponent />
        <li>
          <button type="button" onClick={handleClick}>更新state</button>
        </li>
        <li>
