...
export function ExampleUseEffect () {
  ...
  useEffect(() => {
    ...
    console.log('订阅---document count---', document.getElementById('count').innerText);
    return () => {
      ...
      console.log('取消订阅---count----', count) // 每次执行清除函数，里面获取的数据是旧的
      console.log('取消订阅---document count---', document.getElementById('count').innerText); // 每次执行清除函数，里面获取的dom是最新的（执行下一个 effect 之前，先清除上一个 effect-执行上一个effect的清除函数，传给 useEffect 的函数会在浏览器完成布局与绘制之后，在一个延迟事件中被调用 ）
    };
  }, [count]);

  useLayoutEffect(() => {
    console.log('useLayoutEffect---count----', count)
    console.log('useLayoutEffect---document count---', document.getElementById('count').innerText);
  }, [count])
